/*
  # Criar relacionamentos corretos da tb_usuario com as demais tabelas

  1. Correções de Constraints
    - Adicionar constraint única em tb_usuario.user_id
    - Criar foreign keys corretas para todas as tabelas

  2. Funções Auxiliares
    - is_admin(): Verifica se usuário é admin usando tb_usuario
    - get_user_operator_code(): Retorna código do operador
    - user_is_active(): Verifica se usuário está ativo

  3. Políticas de Segurança
    - Atualizar todas as políticas para usar tb_usuario
    - Manter isolamento de dados por usuário
    - Permitir acesso completo para administradores

  4. Índices de Performance
    - Criar índices condicionais para usuários ativos
*/
...
BEGIN
    IF EXISTS (
        ...
        -- Verificar se existe foreign key para cash_reports
        IF EXISTS (
            SELECT 1 FROM information_schema.table_constraints 
            WHERE constraint_name = 'product_reports_cash_report_id_fkey' 
            AND table_name = 'product_reports'
        ) THEN
            -- Manter a constraint existente, ela já está correta
            NULL;
        END IF;
    END IF;
END $$;

-- Para supply_reports (se existir)
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.tables 
        WHERE table_name = 'supply_reports'
    ) THEN
        -- Verificar se existe foreign key para cash_reports
        IF EXISTS (
            SELECT 1 FROM information_schema.table_constraints 
            WHERE constraint_name = 'supply_reports_cash_report_id_fkey' 
            AND table_name = 'supply_reports'
        ) THEN
            -- Manter a constraint existente, ela já está correta
            NULL;
        END IF;
...
-- Verificar se product_reports existe e habilitar RLS
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.tables 
...
-- Verificar se supply_reports existe e habilitar RLS
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.tables 
